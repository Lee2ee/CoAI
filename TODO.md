...
| # | 항목 | 상태 | 설명 |
|---|------|------|------|
| T1 | 재진입 금지 타이머 | [x] | 손절 종목 2시간 블랙리스트 |
| T2 | 연속 손절 쿨다운 | [x] | 3연속 손절 → 30분 신규 진입 전면 차단 |
| T3 | 손익분기점(BE) SL 이동 | [x] | 수익 +1% 도달 시 SL → 진입가 자동 이동 |

### 🟠 단기 (2주)

| # | 항목 | 상태 | 설명 |
|---|------|------|------|
| T4 | 시간대 필터 | [ ] | 업비트 새벽 01~07시 KST 신규 진입 차단 |
| T5 | 전략-국면 적합성 필터 | [ ] | oversold_bounce는 ranging에서만, macd_momentum은 trending에서만 |
| T6 | 구조적 SL (Swing Low 기반) | [ ] | 고정 % 대신 최근 스윙 저점 아래 SL 배치 |
| T7 | 성과 세분화 추적 | [ ] | 시간대·국면·스타일별 승률·보유시간 분석 |

### 🟡 중기 (1달)

| # | 항목 | 상태 | 설명 |
|---|------|------|------|
| T8 | VWAP 지표 | [ ] | 가격 < VWAP 시 롱 진입 페널티 (-10점) |
| T9 | BB 스퀴즈 감지 | [ ] | 밴드 압축 → 브레이크아웃 예고 신호 |
| T10 | BTC 도미넌스 연동 | [ ] | 도미넌스 상승 중 알트 롱 억제 |
| T11 | 청산 캔들 패턴 | [ ] | 베어리시 인걸핑·슈팅스타 감지 시 즉시 청산 |
| T12 | 분할 진입 (Scale-in) | [ ] | 50% 진입 → 1봉 확인 후 나머지 50% |
| T13 | 유동성 사냥 감지 | [ ] | 스윙 저점 wicking 후 복귀 = 강한 매수 신호 |
| T14 | 리밋 오더 진입 | [ ] | 시장가 대신 0.3% 아래 리밋 → 미체결 시 취소 |

### 완료된 개선 사항 (이번 세션)

| 항목 | 파일 |
|------|------|
| bar-close strategy (확정봉만 사용) | scanner.py |
| ATR 기반 SL 하한 보정 | scanner.py |
| 초단타(5m) 병렬 스캔 | bot.py |
| 시장 국면 무관 기회 진입 (score 75+) | bot.py |
